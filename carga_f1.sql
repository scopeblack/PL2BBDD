...
CREATE TABLE IF NOT EXISTS f1.escuderias(
     escuderíaref        TEXT 
    ,nombre              TEXT
    ,nacionalidad        TEXT
    ,url                 TEXT
...
CREATE TABLE IF NOT EXISTS f1.pilotos_corren_gps(
    ...
    ON DELETE RESTRICT ON UPDATE CASCADE
    ,CONSTRAINT pilotos_corren_gps_fk3 FOREIGN KEY (escuderiaref) REFERENCES f1.escuderias (escuderíaref) MATCH FULL
    ON DELETE RESTRICT ON UPDATE CASCADE
);
...
INSERT INTO f1.escuderias(escuderíaref,nombre,nacionalidad,url)
SELECT
